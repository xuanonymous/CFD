...
! SWIRL 1#################################################
    ...
    DO n=1,nt
        g(n) = 1
            DO j=int((Ax-A)/DX),int((Ro-A)/DX)
                DO i=int((Ax-A)/DY),int((Ro-A)/DY)
                RD(j,i) = MIN(sqrt((x(j)-Bx)**2 + (y(i)-Ax)**2),Ro)/Ro
